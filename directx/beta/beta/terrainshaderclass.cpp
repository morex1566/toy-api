...
TerrainShaderClass::TerrainShaderClass(BaseGameObject* gameObject, string vsRoute, string psRoute)
	: BaseShader(gameObject), m_vsRoute(vsRoute), m_psRoute(psRoute)
{
	bool result;

	m_Name = "TerrainShader";
	...
	if (!result)
	{
		MessageBox(m_GameObject->GetHWND(), L"Could not initialize the terrainshader.", L"Error", MB_OK);
		exit(0);
	}
}
...
bool TerrainShaderClass::Initialize()
{
	bool result;
...
bool TerrainShaderClass::Render(vector<BaseGameObject*> gameObjectList, Camera* camera)
{
	bool result;

	Bitmap* bitmap = dynamic_cast<Bitmap*>(m_GameObject->FindComponentWithName("Bitmap"));
	...
	for (auto light : lightList)
	{
		...
		result = SetShaderParameters(m_GameObject->GetDirectX3D()->GetDeviceContext(),
			...
			light->GetDirection(),
			bitmap->GetGrassTexture(),
			bitmap->GetSlopeTexture(),
			bitmap->GetRockTexture());
		if(!result)
		{
			return false;
		}
		...
		RenderShader(m_GameObject->GetDirectX3D()->GetDeviceContext(),
					 dynamic_cast<Bitmap*>(m_GameObject->FindComponentWithName("Bitmap"))->GetIndexCount());
