...
bool FontShaderClass::Initialize(ID3D11Device* device, HWND hwnd, string vsRoute, string psRoute)
{
	bool result;
	...
	result = InitializeShader(device, hwnd,
							  wstring(vsRoute.begin(), vsRoute.end()).c_str(),
							  wstring(psRoute.begin(), psRoute.end()).c_str());
	if(!result)
	{
		return false;
	}
