...
ReflectionShaderClass::ReflectionShaderClass(BaseGameObject* gameObject, string vsRoute, string psRoute)
	: BaseShader(gameObject), m_VSRoute(vsRoute), m_PSRoute(psRoute)
{
	bool result;

	m_Name = "ReflectionShader";
	...
	if (!result)
	{
		MessageBox(m_GameObject->GetHWND(), L"Could not initialize the ReflectionShader.", L"Error", MB_OK);
		exit(0);
	}
}
...
bool ReflectionShaderClass::Initialize()
{
	bool result;
...
bool ReflectionShaderClass::Render(vector<BaseGameObject*> gameObjectList, Camera* camera)
{
	bool result;
	...
	{
		if (object->GetTag() == "Terrain") 
		{
			terrain = dynamic_cast<Terrain*>(object);
		}
	}
	...
	for (auto light : lightList)
	{
		...
		//	colorTexture, normalTexture, lightDiffuseColor,
