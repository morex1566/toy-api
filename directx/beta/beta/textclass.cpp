...
TextClass::TextClass(D3DClass* directX3D, HWND hwnd, string textRoute, string imageRoute, string vsRoute, string psRoute,
					 int screenWidth, int screenHeight)
	: BaseGameObject(directX3D, hwnd), m_textRoute(textRoute), m_imageRoute(imageRoute), m_vsRoute(vsRoute), m_psRoute(psRoute),
	  m_screenWidth(screenWidth), m_screenHeight(screenHeight)
{
	...
	result = Initialize(directX3D->GetDevice(), directX3D->GetDeviceContext(), hwnd, screenWidth, screenHeight);
	if (!result)
	{
...
bool TextClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	result = m_Font->Initialize(device, m_textRoute.c_str(), m_imageRoute.c_str());
	if(!result)
	{
	...
	result = m_FontShader->Initialize(device, hwnd, m_vsRoute, m_psRoute);
	if(!result)
	{
