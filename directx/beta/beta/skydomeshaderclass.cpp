...
SkyDomeShaderClass::SkyDomeShaderClass(BaseGameObject* gameObject, string vsRoute, string psRoute)
	: BaseShader(gameObject), m_VSRoute(vsRoute), m_PSRoute(psRoute)
{
	bool result;

	m_Name = "SkydomeShader";
	...
	m_apexColor = XMVectorSet(0, 0, 0, 0);
	m_centerColor = XMVectorSet(0, 0, 0, 0);
...
bool SkyDomeShaderClass::Initialize()
{
	bool result;
...
bool SkyDomeShaderClass::Render(vector<BaseGameObject*> gameObjectList, Camera* camera)
{
	bool result;
...
void SkyDomeShaderClass::SetApexColor(XMVECTOR color)
{
	m_apexColor = color;
}

void SkyDomeShaderClass::SetCenterColor(XMVECTOR color)
{
	m_centerColor = color;
}
...
bool SkyDomeShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
											 XMMATRIX projectionMatrix, XMVECTOR apexColor, XMVECTOR centerColor)
{
	HRESULT result;
